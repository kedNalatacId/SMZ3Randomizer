        ...
        public West(World world, Config config) : base(world, config) {
            Locations = new List<Location> {
                ...
                new Location(this, 51, 0x8F8B46, LocationType.Hidden, "Missile (below Ice Beam)", Logic switch {
                    ...
                    _ => new Requirement(items => ((!config.UseKeycards || items.CardNorfairL1) || items.Super) && items.CanUsePowerBombs() && (items.Varia || items.HasEnergyReserves(3)) ||
                        items.Varia && items.SpeedBooster && items.Super && (!config.UseKeycards || items.CardNorfairL1))
                }),
