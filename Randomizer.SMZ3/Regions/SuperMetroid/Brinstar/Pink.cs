        ...
        public Pink(World world, Config config) : base(world, config) {
            ...
            Locations = new List<Location> {
                new Location(this, 14, 0x8F84E4, LocationType.Chozo, "Super Missile (pink Brinstar)", Logic switch {
                    _ => new Requirement(items => (!config.UseKeycards || (items.CardBrinstarBoss || items.CardBrinstarL2)) && items.CanPassBombPassages() && items.Super)
                }),
                ...
                new Location(this, 35, 0x8F8824, LocationType.Visible, "Energy Tank, Brinstar Gate", Logic switch {
                    Normal => items => (!config.UseKeycards || items.CardBrinstarL2) && items.CanUsePowerBombs() && items.Wave && items.HasEnergyReserves(1),
                    _ => new Requirement(items => (!config.UseKeycards || items.CardBrinstarL2) && items.CanUsePowerBombs() && (items.Wave || items.Super))
                }),
            };
        }
