        ...
        public Blue(World world, Config config) : base(world, config) {
            Locations = new List<Location> {
                ...
                new Location(this, 28, 0x8F8798, LocationType.Visible, "Missile (blue Brinstar middle)", Logic switch {
                    _ => new Requirement(items => (!config.UseKeycards || items.CardBrinstarL1) && items.Morph)
                }),
                new Location(this, 29, 0x8F879E, LocationType.Hidden, "Energy Tank, Brinstar Ceiling", Logic switch {
                    Normal => items => (!config.UseKeycards || items.CardBrinstarL1) && (items.CanFly() || items.HiJump ||items.SpeedBooster || items.Ice),
                    _ => new Requirement(items => (!config.UseKeycards || items.CardBrinstarL1))
                }),
                ...
                new Location(this, 36, 0x8F8836, LocationType.Visible, "Missile (blue Brinstar top)", Logic switch {
                    _ => new Requirement(items => (!config.UseKeycards || items.CardBrinstarL1) && items.CanUsePowerBombs())
                }),
