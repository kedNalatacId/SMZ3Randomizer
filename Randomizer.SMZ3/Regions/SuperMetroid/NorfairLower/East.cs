        ...
        public East(World world, Config config) : base(world, config) {
            Locations = new List<Location> {
                ...
                new Location(this, 74, 0x8F8FCA, LocationType.Visible, "Missile (lower Norfair above fire flea room)", Logic switch {
                    _ => new Requirement(items => CanExit(items) && (!config.UseKeycards || items.CardLowerNorfairL1))
                }),
                new Location(this, 75, 0x8F8FD2, LocationType.Visible, "Power Bomb (lower Norfair above fire flea room)", Logic switch {
                    Normal => new Requirement(items => CanExit(items) && (!config.UseKeycards || items.CardLowerNorfairL1)),
                    _ => items => CanExit(items) && (!config.UseKeycards || items.CardLowerNorfairL1) && items.CanPassBombPassages()
                }),
                new Location(this, 76, 0x8F90C0, LocationType.Visible, "Power Bomb (Power Bombs of shame)", Logic switch {
                    _ => new Requirement(items => CanExit(items) && (!config.UseKeycards || items.CardLowerNorfairL1) && items.CanUsePowerBombs())
                }),
                new Location(this, 77, 0x8F9100, LocationType.Visible, "Missile (lower Norfair near Wave Beam)", Logic switch {
                    Normal => new Requirement(items => CanExit(items) && (!config.UseKeycards || items.CardLowerNorfairL1)),
                    _ => items => CanExit(items) && (!config.UseKeycards || items.CardLowerNorfairL1) && items.Morph && items.CanDestroyBombWalls()
                }),
                new Location(this, 78, 0x8F9108, LocationType.Hidden, "Energy Tank, Ridley", Logic switch {
                    _ => new Requirement(items => CanExit(items) && (!config.UseKeycards || (items.CardLowerNorfairL1 && items.CardLowerNorfairBoss)) && items.CanUsePowerBombs() && items.Super)
                }),
