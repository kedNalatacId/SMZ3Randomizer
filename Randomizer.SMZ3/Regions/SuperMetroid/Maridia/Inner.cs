...
namespace Randomizer.SMZ3.Regions.SuperMetroid.Maridia {
    ...
    class Inner : SMRegion, IReward {
        ...
        public Inner(World world, Config config) : base(world, config) {
            Locations = new List<Location> {
                new Location(this, 140, 0x8FC4AF, LocationType.Visible, "Super Missile (yellow Maridia)", Logic switch {
                    Normal => items => (!config.UseKeycards || items.CardMaridiaL1) && items.CanPassBombPassages(),
                    _ => new Requirement(items => (!config.UseKeycards || items.CardMaridiaL1) && items.CanPassBombPassages() &&
                ...
                new Location(this, 144, 0x8FC5DD, LocationType.Visible, "Missile (left Maridia sand pit room)", Logic switch {
                    Normal => items => ((!config.UseKeycards || items.CardMaridiaL1) || ((!config.UseKeycards || items.CardMaridiaL2) && items.CanAccessMaridiaPortal(World))) && items.Super && items.CanPassBombPassages(),
                    _ => new Requirement(items => ((!config.UseKeycards || items.CardMaridiaL1) || ((!config.UseKeycards || items.CardMaridiaL2) && items.CanAccessMaridiaPortal(World))) && items.Super && (items.HiJump && (items.SpaceJump || items.CanSpringBallJump()) || items.Gravity))
                }),
                ...
                new Location(this, 146, 0x8FC5EB, LocationType.Visible, "Missile (right Maridia sand pit room)", Logic switch {
                    Normal => new Requirement(items => ((!config.UseKeycards || items.CardMaridiaL1) || ((!config.UseKeycards || items.CardMaridiaL2) && items.CanAccessMaridiaPortal(World))) && items.Super),
                    _ => items => ((!config.UseKeycards || items.CardMaridiaL1) || ((!config.UseKeycards || items.CardMaridiaL2) && items.CanAccessMaridiaPortal(World))) && items.Super && (items.HiJump || items.Gravity)
                }),
                new Location(this, 147, 0x8FC5F1, LocationType.Visible, "Power Bomb (right Maridia sand pit room)", Logic switch {
                    Normal => new Requirement(items => ((!config.UseKeycards || items.CardMaridiaL1) || ((!config.UseKeycards || items.CardMaridiaL2) && items.CanAccessMaridiaPortal(World))) && items.Super),
                    _ => items => ((!config.UseKeycards || items.CardMaridiaL1) || ((!config.UseKeycards || items.CardMaridiaL2) && items.CanAccessMaridiaPortal(World))) && items.Super && (items.HiJump && items.CanSpringBallJump() || items.Gravity)
                }),
                new Location(this, 148, 0x8FC603, LocationType.Visible, "Missile (pink Maridia)", Logic switch {
                    Normal => items =>((!config.UseKeycards || items.CardMaridiaL1) || ((!config.UseKeycards || items.CardMaridiaL2) && items.CanAccessMaridiaPortal(World))) &&  items.SpeedBooster,
                    _ => new Requirement(items => ((!config.UseKeycards || items.CardMaridiaL1) || ((!config.UseKeycards || items.CardMaridiaL2) && items.CanAccessMaridiaPortal(World))) && items.Gravity)
                }),
                ...
                new Location(this, 151, 0x8FC74D, LocationType.Hidden, "Missile (Draygon)", Logic switch {
                    Normal => items => ((!config.UseKeycards || (items.CardMaridiaL1 && items.CardMaridiaL2)) && CanDefeatBotwoon(items)) || items.CanAccessMaridiaPortal(World),
                    _ => new Requirement(items => (((!config.UseKeycards || (items.CardMaridiaL1 && items.CardMaridiaL2)) && CanDefeatBotwoon(items)) || items.CanAccessMaridiaPortal(World)) && items.Gravity)
                }),
                new Location(this, 152, 0x8FC755, LocationType.Visible, "Energy Tank, Botwoon", Logic switch {
                    _ => new Requirement(items => ((!config.UseKeycards || (items.CardMaridiaL1 && items.CardMaridiaL2)) && CanDefeatBotwoon(items)) || (items.CanAccessMaridiaPortal(World) && (!config.UseKeycards || items.CardMaridiaL2)))
                }),
        ...
        bool CanDefeatDraygon(Progression items) {
            return Logic switch {
                Normal => (((!Config.UseKeycards || (items.CardMaridiaL1 && items.CardMaridiaL2)) && CanDefeatBotwoon(items)) || items.CanAccessMaridiaPortal(World))  && (!Config.UseKeycards || items.CardMaridiaBoss) &&
                    items.Gravity && (items.SpeedBooster && items.HiJump || items.CanFly()),
                _ => (((!Config.UseKeycards || (items.CardMaridiaL1 && items.CardMaridiaL2)) && CanDefeatBotwoon(items)) || items.CanAccessMaridiaPortal(World)) && (!Config.UseKeycards || items.CardMaridiaBoss) &&
                    items.Gravity
            };
        }
