        ...
        public override bool CanEnter(Progression items) {
            return Logic switch {
                Normal => items.Gravity && (
                        ...
                        items.CanAccessMaridiaPortal(World) && (!Config.UseKeycards || (items.CardMaridiaL1 && items.CardMaridiaL2)) && (items.CanPassBombPassages() || items.ScrewAttack)
                    ),
                _ =>
                    World.CanEnter("Norfair Upper West", items) && items.CanUsePowerBombs() &&
                        (items.Gravity || items.HiJump && (items.CanSpringBallJump() || items.Ice)) ||
                    items.CanAccessMaridiaPortal(World) && (!Config.UseKeycards || (items.CardMaridiaL1 && items.CardMaridiaL2)) && (
